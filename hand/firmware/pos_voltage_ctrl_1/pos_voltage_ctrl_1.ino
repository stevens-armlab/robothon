...
void messageCb(const sensor_msgs::Joy& data) {
  ...
  spread_cmd = data.buttons[0] - data.buttons[1];  //<-- --> spread / unspread
}
...
ros::Publisher pub_load("load", &load_msg);
...
void grasping() {
  ...
  if (dxl_1.getPresentPosition(DXL_ID_1, UNIT_RAW) >= pos_max_1)
  {
    motor_states[0] = 3;
    if(!pos_ctrl)
    {
      ...
      pos_ctrl = true;
      dxl_1.setGoalPosition(DXL_ID_1, pos_max_1, UNIT_RAW);
    }
  ...
  load_pre = dxl_1.readControlTableItem(PRESENT_LOAD, DXL_ID_1);
  if (load_pre < 90.0) //Constant Velocity
  {
    if(!pos_ctrl)
    {
      dxl_1.torqueOff(DXL_ID_1);
    ...
    dxl_1.setGoalPosition(DXL_ID_1, dxl_1.getPresentPosition(DXL_ID_1, UNIT_RAW) + 20, UNIT_RAW);
    delay(10);
    motor_states[0] = 1;
  }
  else
  {
    ...
    dxl_1.setGoalPWM(DXL_ID_1, 30, UNIT_PERCENT);
    motor_states[0] = 2;
  }
}
  ...
  */
  ...
  else
  {
    if(!pos_ctrl)
    {
      ...
      dxl_1.torqueOn(DXL_ID_1);
      pos_ctrl = true;
      dxl_1.setGoalPosition(DXL_ID_1, dxl_1.getPresentPosition(DXL_ID_1, UNIT_RAW), UNIT_RAW);
    }
  }
  if (open_cmd == 1) {
    opening();
