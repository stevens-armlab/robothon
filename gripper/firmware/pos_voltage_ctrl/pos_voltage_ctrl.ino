...
const uint8_t DXL_ID_1 = 12; // Motor Specific
const float DXL_PROTOCOL_VERSION_1 = 2.0; // Motor Specific
const int32_t BAUD_RATE_MOTOR = 1000000; // Motor Specific
...
void gripper_grasp_set_position_cb(const std_msgs::Float64& data);
void gripper_grasp_stop_cb(const std_msgs::Bool& data);
void gripper_grasp_start_grasp_cb(const gripper::grasp& data);
void grasp();
float filter();
void initialization();
void motor_setup();
...
ros::Subscriber<std_msgs::Float64> grasp_set_pos_sub("gripper/grasp/set_position", &gripper_grasp_set_position_cb);
ros::Subscriber<gripper::grasp> start_grasp_sub("gripper/grasp/start_grasp", &gripper_grasp_start_grasp_cb);
ros::Subscriber<std_msgs::Bool> stop_grasp_sub("gripper/grasp/stop", &gripper_grasp_stop_cb);

ros::Publisher grasp_state_pub("gripper/grasp/state", &grasp_state_msg);
ros::Publisher grasp_load_pub("gripper/grasp/load", &load_msg);
ros::Publisher grasp_position_pub("gripper/grasp/position", &grasp_position_msg);
